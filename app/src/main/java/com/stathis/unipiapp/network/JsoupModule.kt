    ...
    fun getUnipiServices(data : MutableLiveData<List<UnipiService>>,error : MutableLiveData<Boolean>){
        ...
        try {
            ...
            for (i in 0..9) {
                ...
                val image = doc.select(".itemList").select(".itemContainer").select(".catItemView").select(".catItemBody").select(".catItemImageBlock").select("span.catItemImage").select("a").select("img").eq(i).attr("src")
